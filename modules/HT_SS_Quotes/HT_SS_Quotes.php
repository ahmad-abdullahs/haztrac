...
require_once($beanFiles['HT_Sales_Service_Products']);
require_once($beanFiles['HT_SS_ProductBundles']);

require_once('modules/HT_SS_Quotes/HT_SS_Quotes_sugar.php');
class HT_SS_Quotes extends HT_SS_Quotes_sugar {
    /**
    ...
    public $rel_account_table = "ht_quotes_accounts";
    public $rel_contact_table = "ht_quotes_contacts";
    public $rel_opportunity_table = "ht_quotes_opportunities";
    ...
    public $rel_product_bundles = 'ht_product_bundle_quote';
    ...
    /**
     * Delete all the associated HT_Sales_Service_Products Bundles from a Quote, This ensures that no orphaned records are left behind,
    ...
    public function mark_deleted($id)
    {
        ...
        /* @var $bundle HT_SS_ProductBundles */
    ...
    public function build_generic_where_clause($the_query_string)
    {
        ...
        array_push($where_clauses, "ht_ss_quotes.name like " . $this->db->quoted($the_query_string. "%"));
    ...
    public function get_product_bundles()
    {
        ...
        $prodBundlesBean = BeanFactory::newBean('HT_SS_ProductBundles');
        $query->select('*');
    ...
    public function get_account_detail($quote_id)
    {
        ...
        $query = "SELECT acc.id, acc.name, acc.assigned_user_id "
            . "FROM accounts acc, {$this->rel_account_table} a_q "
    ...
    {
        $results = array();
        $query = "select * from {$this->rel_opportunity_table} where quote_id = ? and deleted = 0";
    ...
    {
        $query = "select count(id) from {$this->rel_opportunity_table} where quote_id = ? and deleted = 0";
        $conn = $this->db->getConnection();
    ...
    /**
     * This defines the supporting modules which have metadata needed by HT_SS_Quotes to be fully
    ...
    public static function getMobileSupportingModules()
    {
        ...
        return array_merge($modules, array(
            ...
            'ProductTypes',
            'ProductCategories',
            'HT_Sales_Service_Catalog',
            'HT_SS_ProductBundles',
            'HT_SS_ProductBundlesNotes',
            'Manufacturers',
            'TaxRates',
            'Shippers',
        ));
