 ...
 */
/**
 * @class View.Layouts.Base.HT_SS_Quotes.QuoteDataListGroupsLayout
...
({
    ...
    initialize: function(options) {
        ...
        this.quoteDataGroupMeta = app.metadata.getLayout('HT_SS_ProductBundles', 'quote-data-group');
    ...
    bindDataChange: function() {
        ...
        this.context.on('ht_ss_quotes:group:create', this._onCreateQuoteGroup, this);
        this.context.on('ht_ss_quotes:group:delete', this._onDeleteQuoteGroup, this);
        this.context.on('ht_ss_quotes:selected:delete', this._onDeleteSelectedItems, this);
        this.context.on('ht_ss_quotes:defaultGroup:create', this._onCreateDefaultQuoteGroup, this);
        this.context.on('ht_ss_quotes:defaultGroup:save', this._onSaveDefaultQuoteGroup, this);

        if (!(_.has(userACLs.HT_SS_Quotes, 'edit') ||
                _.has(userACLs.HT_Sales_Service_Products, 'access') ||
                _.has(userACLs.HT_Sales_Service_Products, 'edit'))) {
            // only listen for PCDashlet if this is HT_SS_Quotes and user has access
            // to both HT_SS_Quotes and HT_Sales_Service_Products
    ...
    toggleCopyAlert: function(showAlert) {
        ...
        if (showAlert) {
            ...
            app.alert.show(alertId, {
                ...
                closeable: false,
                autoClose: false,
                title: app.lang.get(titleLabel, 'HT_SS_Quotes')
            });
        } else {
    ...
    _setCopyQuoteData: function() {
        ...
        _.each(relatedRecords, function(record) {
            ...
            if (record.default_group) {
                _.each(record.ht_product_bundle_items, function(pbItem) {
                ...
                defaultGroup.quoteDataGroupList.collection.add(record.ht_product_bundle_items);
                ...
                this.context.once(
                    'ht_ss_quotes:group:create:success',
    ...
    _onCopyQuoteDataNewGroupedCreateSuccess: function(record, pbModel) {
        ...
        // add the whole collection of PBItems to the list collection at once
        group.quoteDataGroupList.collection.add(record.ht_product_bundle_items);

        // update the copy bundle number
        this.completedCopyItem(true);

        // update the group line number counts
        group.trigger('ht_ss_quotes:line_nums:reset');
    },
    ...
    _onProductCatalogDashletAddItem: function(productData) {
        ...
        if (defaultGroup) {
            ...
            defaultGroup.trigger('ht_ss_quotes:group:create:qli', 'products', productData);
        }
    ...
    /**
     * Checks all HT_Sales_Service_Products in bundles to make sure each Product has quote_id set
     *
     * @private
     */
    _checkProductsQuoteLink: function() {
        ...
        _.each(bundles.models, function(pbModel) {
            pbItems = pbModel.get('ht_product_bundle_items');

            _.each(pbItems.models, function(itemModel) {
                if (itemModel.module === 'HT_Sales_Service_Products') {
                    ...
                    if (prodId && _.isEmpty(itemModel.get('quote_id'))) {
                        bulkUrl = app.api.buildURL('HT_Sales_Service_Products/' + prodId + '/link/ht_ss_quotes/' + quoteId);
                        bulkRequest = {
            ...
            app.api.call('create', app.api.buildURL(null, 'bulk'), {
                ...
                success: _.bind(function(bulkResponses) {
                    _.each(bulkResponses, function(response) {
                        ...
                        _.each(bundles.models, function(pbModel) {
                            var pbItems = pbModel.get('ht_product_bundle_items');
                            _.each(pbItems.models, function(itemModel) {
    ...
    /**
     * Handles when the show_line_nums attrib changes on the HT_SS_Quotes model, triggers if
     * line numbers should be shown or not
     *
     * @param {Data.Bean} model The HT_SS_Quotes Bean the change happened on
     * @param {boolean} showLineNums If the line nums should be shown or not
     * @private
     */
    _onShowLineNumsChanged: function(model, showLineNums) {
        this.context.trigger('ht_ss_quotes:show_line_nums:changed', showLineNums);
    },

    /**
     * Handles the ht_ss_quotes:defaultGroup:create event from a separate layout context
    ...
    _onCreateDefaultQuoteGroup: function(itemType) {
        ...
        var linkName = itemType == 'qli' ? 'products' : 'ht_product_bundle_notes';
        var group = this._getComponentByGroupId(this.defaultGroupId);
        group.trigger('ht_ss_quotes:group:create:' + itemType, linkName);
    },
    ...
    _onDragStop: function(evt, ui) {
        ...
        if (!this.isCreateView && !_.isEmpty(this.currentBulkSaveRequests)) {
            if (triggerOldGroup) {
                ...
                oldGroup.trigger('ht_ss_quotes:group:changed');
                // trigger save start for the old group
                oldGroup.trigger('ht_ss_quotes:group:save:start');
                // trigger the group to reset it's line numbers
                oldGroup.trigger('ht_ss_quotes:line_nums:reset', oldGroup.groupId, oldGroup.collection);
            }

            // trigger group changed for new group to check themselves
            newGroup.trigger('ht_ss_quotes:group:changed');
            // trigger save start for the new group
            newGroup.trigger('ht_ss_quotes:group:save:start');
            // trigger the group to reset it's line numbers
            newGroup.trigger('ht_ss_quotes:line_nums:reset', newGroup.groupId, newGroup.collection);
    ...
    _updateModelWithRecord: function(model, record) {
        if (model) {
            ...
            if (record.hasOwnProperty('ht_product_bundle_items') && _.isEmpty(record.ht_product_bundle_items)) {
                delete record.ht_product_bundle_items;
            }
    ...
    _moveItemToNewGroup: function(oldGroupId, newGroupId, itemId, isRowInEdit, newPos, updateLinkBean, updatePos) {
        ...
        linkName = rowModel.module === 'HT_Sales_Service_Products' ? 'products' : 'ht_product_bundle_notes';
        url = app.api.buildURL('HT_SS_ProductBundles/' + newGroupModelId + '/link/' + linkName + '/' + itemModelId);
        bulkMoveRequest = {
            url: url.substr(4),
            method: 'POST',
            data: {
        ...
        url = app.api.buildURL('HT_SS_ProductBundles/' + oldGroupModelId);
        bulkMoveRequest = {
            url: url.substr(4),
            method: 'GET'
        };
    ...
    _onSaveDefaultQuoteGroup: function(successCallback) {
        ...
        app.alert.show('saving_default_group_alert', {
            level: 'success',
            autoClose: false,
            messages: app.lang.get('LBL_SAVING_DEFAULT_GROUP_ALERT_MSG', 'HT_SS_Quotes')
        });

        app.api.relationships('create', 'HT_SS_Quotes', {
    ...
    _onSaveUpdatedGroupSuccess: function(oldGroup, newGroup, bulkResponses) {
        ...
        if (oldGroup) {
            oldGroup.trigger('ht_ss_quotes:group:save:stop');
        }
        newGroup.trigger('ht_ss_quotes:group:save:stop');
        ...
        if (deletedGroupId) {
            ...
            app.alert.show('deleted_bundle_alert', {
                level: 'success',
                autoClose: true,
                messages: app.lang.get('LBL_DELETED_BUNDLE_SUCCESS_MSG', 'HT_SS_Quotes')
            });

            // get the deleted group
            deletedGroup = this._getComponentByGroupId(deletedGroupId);
            // get the bundle for the deleted group
            deletedGroupBundle = deletedGroup.model.get('ht_product_bundle_items');
            // get the bundle for the new group
            newGroupBundle = newGroup.model.get('ht_product_bundle_items');
        ...
        _.each(bulkResponses, _.bind(function(oldGroup, newGroup, data) {
            ...
            // on GET HT_SS_ProductBundles relatedRecord will not exist but isGetRequest should be set above
    ...
    _updateRowPositions: function(dataGroup) {
        ...
        _.each(rows, _.bind(function(dataGroup, retObj, row, index) {
            ...
            if (rowModel.get('position') != index && !rowModel.isNew()) {
                ...
                linkName = rowModule === 'HT_Sales_Service_Products' ? 'products' : 'ht_product_bundle_notes';
                url = app.api.buildURL('HT_SS_ProductBundles/' + dataGroupModelId + '/link/' + linkName + '/' + itemModelId);
                retCalls.push({
                    url: url.substr(4),
                    method: 'PUT',
                    data: {
    ...
    _onGroupDragTriggerOver: function(evt, ui) {
        ...
        if (group) {
            group.trigger('ht_ss_quotes:sortable:over', evt, ui);
        }
    },
    ...
    _onGroupDragTriggerOut: function(evt, ui) {
        ...
        var group = this._getComponentByGroupId(groupId);
        if (group) {
            group.trigger('ht_ss_quotes:sortable:out', evt, ui);
        }
    },
    ...
    /**
     * Creates the default HT_SS_ProductBundles Bean with default group ID
     *
     * @return {Data.Bean}
     * @protected
     */
    ...
    _createNewProductBundleBean: function(groupId, newPosition, isDefaultGroup) {
        ...
        return app.data.createBean('HT_SS_ProductBundles', {
            _module: 'HT_SS_ProductBundles',
            _action: 'create',
            ...
            ht_product_bundle_items: [],
            ht_product_bundle_notes: [],
    ...
    _addQuoteGroupToLayout: function(bundle) {
        ...
        var groupLayout = app.view.createLayout({
            ...
            layout: this,
            module: 'HT_SS_ProductBundles',
            model: bundle
        });

        groupLayout.initComponents(undefined, pbContext, 'HT_SS_ProductBundles');
        this.addComponent(groupLayout);
    },

    /**
     * Handles the ht_ss_quotes:group:create event
    ...
    _onCreateQuoteGroup: function() {
        ...
        if (this.isCreateView) {
            ...
            this.context.trigger('ht_ss_quotes:group:create:success', newBundle);
        } else {
            app.alert.show('adding_bundle_alert', {
                level: 'info',
                autoClose: false,
                messages: app.lang.get('LBL_ADDING_BUNDLE_ALERT_MSG', 'HT_SS_Quotes')
            });
    ...
    _onCreateQuoteGroupSuccess: function(newBundleData) {
        ...
        app.alert.show('added_bundle_alert', {
            level: 'success',
            autoClose: true,
            messages: app.lang.get('LBL_ADDED_BUNDLE_SUCCESS_MSG', 'HT_SS_Quotes')
        });

        var bundles = this.model.get('bundles');
        // make sure that the ht_product_bundle_items array is there
        if (_.isUndefined(newBundleData.related_record.ht_product_bundle_items)) {
            newBundleData.related_record.ht_product_bundle_items = [];
        }
        ...
        if (this.model.get('show_line_nums')) {
            ...
            this.context.trigger('ht_ss_quotes:show_line_nums:changed', true);
        }

        // trigger that the group create was successful and pass the new group data
        this.context.trigger('ht_ss_quotes:group:create:success', newBundleData);
    },
    ...
    _onDeleteSelectedItems: function(massCollection) {
        ...
        _.each(groupsToUpdate, function(groupIdToUpdate) {
            url = app.api.buildURL('HT_SS_ProductBundles' + '/' + groupIdToUpdate);
            bulkRequests.push({
                url: url.substr(4),
                method: 'GET'
            });
    ...
    _onDeleteSelectedItemsSuccess: function(massCollection, bulkRequests) {
        ...
        _.each(bulkRequests, function(request) {
            ...
            if (model) {
                ...
                groupLayout.trigger('ht_ss_quotes:line_nums:reset', groupLayout.groupId, groupLayout.collection);
            }
        }, this);
    },
    ...
    _onDeleteQuoteGroup: function(groupToDelete) {
        ...
        app.alert.show('confirm_delete_bundle', {
            level: 'confirmation',
            autoClose: false,
            messages: app.lang.get('LBL_DELETING_BUNDLE_CONFIRM_MSG', 'HT_SS_Quotes', {
                groupName: groupName
            }),
    ...
    _onDeleteQuoteGroupConfirm: function(groupId, groupName, groupToDelete) {
        ...
        app.alert.show('deleting_bundle_alert', {
            level: 'info',
            autoClose: false,
            messages: app.lang.get('LBL_DELETING_BUNDLE_ALERT_MSG', 'HT_SS_Quotes', {
                groupName: groupName
            })
        });
        ...
        if (this.isCreateView) {
            ...
            if (groupToDelete.model && groupToDelete.model.has('ht_product_bundle_items')) {
                bundleItems = groupToDelete.model.get('ht_product_bundle_items');
            }
            ...
            if (defaultGroup.model && defaultGroup.model.has('ht_product_bundle_items')) {
                positionStart = defaultGroup.model.get('ht_product_bundle_items').length;
            }

            if (bundleItems && bundleItems.length > 0) {
                _.each(bundleItems.models, _.bind(function(groupId, bulkRequests, posStart, model, index, list) {
                    linkName = (model.module === 'HT_Sales_Service_Products' ? 'products' : 'ht_product_bundle_notes');
                    url = app.api.buildURL('HT_SS_ProductBundles/' + groupId + '/link/' +
                        linkName + '/' + model.id);
            ...
            url = app.api.buildURL('HT_SS_ProductBundles/' + groupId);
