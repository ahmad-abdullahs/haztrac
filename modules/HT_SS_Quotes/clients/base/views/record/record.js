 ...
 */
/**
 * @class View.Views.Base.HT_SS_Quotes.RecordView
...
({
    ...
    /**
     * registers additional editable fields from supporting ht_ss_quotes views
     */
    ...
    initialize: function(options) {
        this.plugins = _.union(this.plugins || [], ['HistoricalSummary', 'HTQuotesViewSaveHelper']);
    ...
    bindDataChange: function() {
        ...
        this.context.on('ht_ss_quotes:editableFields:add', function(field) {
            this.additionalEditableFields.push(field);
            this.editableFields.push(field);
        }, this);

        this.context.on('ht_ss_quotes:item:toggle', this._handleItemToggled, this);
    },
     ...
     * HT_SS_Quotes->HT_SS_ProductBundles->HT_Sales_Service_Products|HT_SS_ProductBundlesNotes data structure
     */
    duplicateClicked: function() {
        ...
        if (this.editCount) {
            app.alert.show('quotes_qli_editmode', {
                level: 'error',
                title: '',
                messages: [app.lang.get('LBL_COPY_LINE_ITEMS', 'HT_SS_Quotes')]
            });
        ...
        _.each(bundles.models, function(bundle) {
            ...
            _.each(pbItems, function(pbItem) {
                ...
                if (_.isEmpty(tmpItem.ht_sales_service_catalog_name)) {
                    // if ht_sales_service_catalog_name is empty, use the QLI's name
                    tmpItem.ht_sales_service_catalog_name = tmpItem.name;
                } else {
                    // if ht_sales_service_catalog_name is not empty, set that to the QLI's name
                    tmpItem.name = tmpItem.ht_sales_service_catalog_name;
                }
                ...
                // creates a Bean and pushes the individual HT_Sales_Service_Products|HT_SS_ProductBundlesNotes to the array
                items.push(newBean);
            }, this);
            ...
            bundleData.ht_product_bundle_items = items;
        ...
        // lead the HT_SS_Quotes create layout
        app.controller.loadView(loadViewObj);
        // update the browser URL with the proper
        app.router.navigate('#HT_SS_Quotes/create', {trigger: false});
    },
    ...
    saveClicked: function() {
        ...
        if (this.editCount == 0) {
            ...
            app.alert.show('quotes_qli_editmode', {
                level: 'error',
                title: '',
                messages: [app.lang.get('LBL_SAVE_LINE_ITEMS', 'HT_SS_Quotes')]
            });
        }
