 ...
 */
/**
 * @class View.Views.Base.HT_SS_Quotes.CreateView
...
({
    ...
    /**
     * Holds the HT_SS_ProductBundles/HT_Sales_Service_Products/HT_SS_ProductBundlesNotes fields meta for different views
     */
    ...
    /**
     * If this Create view is from converting items from other modules to HT_SS_Quotes, is this
    ...
    initialize: function(options) {
        this.plugins = _.union(this.plugins || [], ['HTQuotesViewSaveHelper', 'LinkedModel']);
        ...
        this._buildMeta('HT_SS_ProductBundlesNotes', 'quote-data-group-list');
        this._buildMeta('HT_SS_ProductBundles', 'quote-data-group-header');
        this._buildMeta('HT_Sales_Service_Products', 'quote-data-group-list');
    ...
    /**
     * Validates the models in the Quote's HT_SS_ProductBundles
     *
    ...
    validateBundleModels: function(fields, recordErrors, callback) {
        ...
        if (bundles && bundles.length) {
            ...
            if (bundles.length === 1) {
                productBundleItems = bundles.models[0].get('ht_product_bundle_items');
            ...
            totalItemsToValidate = _.reduce(bundles.models, function(memo, bundle) {
                return memo + bundle.get('ht_product_bundle_items').length;
            }, totalItemsToValidate);

            // loop through each HT_SS_ProductBundles bean
            _.each(bundles.models, function(bundleModel) {
                ...
                bundleModel.isValidAsync(this.moduleFieldsMeta[bundleModel.module], _.bind(function(isValid, errors) {
                    ...
                    productBundleItems = bundleModel.get('ht_product_bundle_items');
                    ...
                    // loop through each ht_product_bundle_items HT_Sales_Service_Products/HT_SS_ProductBundlesNotes bean
                    _.each(productBundleItems.models, function(pbModel) {
                        ...
                        pbModel.isValidAsync(this.moduleFieldsMeta[pbModel.module], _.bind(function(isValid, errors) {
                            ...
                            if (errors.description) {
                                // if this is a HT_SS_ProductBundlesNotes model where "description" field is required
    ...
    /**
     * Overriding to make the router go back to previous view, not HT_SS_Quotes module list
     *
     * @inheritdoc
     */
    cancel: function() {
    ...
    getCustomSaveOptions: function(options) {
        ...
        if (isConvert) {
            _.each(bundles.models, function(bundle) {
                var pbItems = bundle.get('ht_product_bundle_items');
                _.each(pbItems.models, function(itemModel) {
                    if (itemModel.module === 'HT_Sales_Service_Products' && itemModel.get('revenuelineitem_id')) {
                        hasItems++;
                    }
                }, this);
            }, this);
        ...
        _.each(bundles.models, function(pbModel) {
            ...
            _.each(pbItems.models, function(itemModel) {
                if (itemModel.module === 'HT_Sales_Service_Products') {
                    ...
                    if (rliId) {
                        bulkUrl = app.api.buildURL('RevenueLineItems/' + rliId + '/link/ht_ss_quotes/' + quoteId);
                        bulkRequest = {
