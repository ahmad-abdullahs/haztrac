 ...
 */
/**
 * @class View.Views.Base.HT_SS_Quotes.QuoteDataListHeaderView
...
({
    ...
    initialize: function(options) {
        ...
        var qliListMetadata = app.metadata.getView('HT_Sales_Service_Products', 'quote-data-group-list');
    ...
    _render: function() {
        ...
        if (this.massCollection) {
            // remove any HT_SS_Quotes models from the massCollectio
            this.massCollection.models = _.filter(this.massCollection.models, function(model) {
                return model.module !== 'HT_SS_Quotes';
            });
        }
    ...
    checkAll: function(event) {
        ...
        if ($checkbox.is(':checked')) {
            this.context.trigger('ht_ss_quotes:collections:all:checked');
        } else {
            this.context.trigger('ht_ss_quotes:collections:not:all:checked');
        }
    },
    ...
    _checkMassActions: function() {
        ...
        quoteModel = _.find(this.massCollection.models, function(model) {
            return model.get('_module') === 'HT_SS_Quotes';
        });

        if (quoteModel) {
            // get rid of any HT_SS_Quotes models from the mass collection
    ...
    _bundlesAreEmpty: function() {
        ...
        if (bundles) {
            bundlesHaveItems = bundles.every(function(bundle) {
                return bundle.get('ht_product_bundle_items').length === 0;
            });
        }
    ...
    _onCreateGroupBtnClicked: function(evt) {
        if (this.massCollection.length) {
            this.context.on('ht_ss_quotes:group:create:success', this._onNewGroupedItemsCreateSuccess, this);
            this.context.trigger('ht_ss_quotes:group:create');
        } else {
    ...
    _onNewGroupedItemsCreateSuccess: function(newGroupData) {
        this.context.off('ht_ss_quotes:group:create:success', this._onNewGroupedItemsCreateSuccess);
        ...
        if (this.massCollection.length) {
            ...
            app.alert.show('confirm_delete', {
                ...
                onConfirm: _.bind(function() {
                    ...
                    this.context.trigger('ht_ss_quotes:selected:delete', this.massCollection);
                }, this)
            });
        } else {
            app.alert.show('quote_grouping_message', {
    ...
    _dispose: function() {
        ...
        this.context.off('ht_ss_quotes:group:create:success', null, this);
