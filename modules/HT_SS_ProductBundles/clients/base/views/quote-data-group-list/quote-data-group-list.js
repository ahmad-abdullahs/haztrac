 ...
 */
/**
 * @class View.Views.Base.HT_SS_ProductBundles.QuoteDataGroupListView
 * @alias SUGAR.App.view.views.BaseHT_SS_ProductBundlesQuoteDataGroupListView
...
({
    ...
    /**
     * HT_SS_ProductBundlesNotes QuoteDataGroupList metadata
     */
    ...
    /**
     * HT_SS_ProductBundlesNotes Description field metadata
     */
    ...
    initialize: function(options) {
        ...
        this.pbnListMetadata = app.metadata.getView('HT_SS_ProductBundlesNotes', 'quote-data-group-list');
        this.qliListMetadata = app.metadata.getView('HT_Sales_Service_Products', 'quote-data-group-list');
        ...
        // get the ht_product_bundle_items collection from the model
        options.collection = options.model.get('ht_product_bundle_items');

        // use the same massCollection from the HT_SS_Quotes QuoteDataListHeaderView
        ...
        this.isOppsConvert = this.isCreateView &&
            ...
            this.context.parent.get('fromLink') != 'ht_ss_quotes';
        ...
        var collections = this.model.fields['ht_product_bundle_items'].links;
        ...
        this.layout.on('ht_ss_quotes:group:create:qli', this.onAddNewItemToGroup, this);
        this.layout.on('ht_ss_quotes:group:create:note', this.onAddNewItemToGroup, this);
        this.layout.on('ht_ss_quotes:sortable:over', this._onSortableGroupOver, this);
        this.layout.on('ht_ss_quotes:sortable:out', this._onSortableGroupOut, this);
        ...
        this.context.parent.on('ht_ss_quotes:collections:all:checked', this.onAllChecked, this);
        this.context.parent.on('ht_ss_quotes:collections:not:all:checked', this.onNotAllChecked, this);
    ...
    onCancelRowEdit: function(rowModel) {
        ...
        if (rowModel.isNew()) {
            ...
            if (this.showLineNums && rowModel.module === 'HT_Sales_Service_Products') {
    ...
    onSaveRowEdit: function(rowModel) {
        ...
        if (rowModel.module === 'HT_Sales_Service_Products' && _.isUndefined(quoteId)) {
            quoteModel = this.context.get('parentModel');

            if (quoteModel) {
                quoteId = quoteModel.get('id');

                app.api.relationships('create', 'HT_Sales_Service_Products', {
                    id: productId,
                    link: 'ht_ss_quotes',
                    relatedId: quoteId,
                    related: {
                    ...
                    success: _.bind(function(response) {
                        ...
                        var pbItems = this.model.get('ht_product_bundle_items');
     ...
     *
     * @param {Data.Bean} groupModel The HT_SS_ProductBundles model
    ...
    onAddNewItemToGroup: function(linkName, prepopulateData) {
        ...
        var position = 0;
        var $relatedRow;
        var moduleName = linkName === 'products' ? 'HT_Sales_Service_Products' : 'HT_SS_ProductBundlesNotes';
        var modelData = {};
        var groupLineNumObj;
        ...
        if (prepopulateData && prepopulateData._forcePosition) {
            ...
            _.each(this.collection.models, function(model) {
                ...
                if (this.showLineNums && model.module === 'HT_Sales_Service_Products') {
            ...
            if (this.showLineNums && relatedModel.module === 'HT_Sales_Service_Products') {
        ...
        app.alert.show('confirm_delete', {
            ...
            onConfirm: _.bind(function() {
                app.alert.show('deleting_line_item', {
                    level: 'info',
                    messages: [app.lang.get('LBL_ALERT_DELETING_ITEM', 'HT_SS_ProductBundles')]
                });
        ...
        deletedRowModel.destroy({
            success: _.bind(function() {
                ...
                app.alert.show('deleted_line_item', {
                    level: 'success',
                    autoClose: true,
                    messages: app.lang.get('LBL_DELETED_LINE_ITEM_SUCCESS_MSG', 'HT_SS_ProductBundles')
                });
            }, this)
        });
        this.layout.trigger('ht_ss_quotes:line_nums:reset', this.layout.groupId, this.layout.collection);
    },
    ...
    toggleRow: function(rowModule, rowModelId, isEdit) {
        ...
        this.context.parent.trigger('ht_ss_quotes:item:toggle', isEdit, rowModelId);
    ...
    getFieldNames: function(module) {
        ...
        if (module === 'HT_SS_Quotes' || module === 'HT_Sales_Service_Products') {
            panels = _.clone(this.qliListMetadata.panels);
        } else if (module === 'HT_SS_ProductBundlesNotes') {
    ...
    _dispose: function() {
        if (this.context && this.context.parent) {
            this.context.parent.off('ht_ss_quotes:collections:all:checked', null, this);
            this.context.parent.off('ht_ss_quotes:collections:not:all:checked', null, this);
        }
