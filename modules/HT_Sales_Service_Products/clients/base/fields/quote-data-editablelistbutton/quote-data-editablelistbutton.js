 ...
 */
/**
 * @class View.Fields.Base.HT_Sales_Service_Products.EditablelistbuttonField
...
({
    ...
    saveClicked: function(evt) {
        // If name exists but ht_sales_service_catalog_name is empty,
        // copy name to ht_sales_service_catalog_name so the field validates
        if (!_.isEmpty(this.model.get('name')) && _.isEmpty(this.model.get('ht_sales_service_catalog_name'))) {
            this.model.set('ht_sales_service_catalog_name', this.model.get('name'), {silent: true});
        }
    ...
    _save: function() {
        ...
        if (this.view.model.isNew()) {
            this.view.context.parent.trigger('ht_ss_quotes:defaultGroup:save', _.bind(this._saveRowModel, this));
        } else {
            this._saveRowModel();
        }
    },
    ...
    _saveRowModel: function() {
        ...
        var successCallback = function(model) {
            ...
            if (self.view.layout) {
                ...
                self.view.layout.trigger('ht_ss_quotes:line_nums:reset');
            }
            ...
            if (model.collection._resavePositions) {
                ...
                _.each(model.collection.models, function(mdl) {
                    itemModelId = mdl.id;
                    linkName = mdl.module === 'HT_Sales_Service_Products' ? 'ht_sales_service_products' : 'product_bundle_notes';
                    bulkUrl = app.api.buildURL('HT_SS_ProductBundles/' + collectionId + '/link/' +
