 ...
 */
require_once('modules/HT_Sales_Service_Products/HT_Sales_Service_Products_sugar.php');
class HT_Sales_Service_Products extends HT_Sales_Service_Products_sugar {
	CONST STATUS_CONVERTED_TO_QUOTE = 'Converted to Quote';

    const STATUS_QUOTED = 'HT_SS_Quotes';
    ...
    public $name;
    public $ht_sales_service_catalog_id;
    public $description;
    ...
    public $rel_types = "ht_product_types";
    public $rel_products = "ht_product_ht_product";
    public $rel_categories = "ht_product_categories";
    ...
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::fill_in_additional_list_fields() has been deprecated in 7.8');
     ...
     *
     * get_list_view_data
     * Returns a list view of the associated HT_Sales_Service_Products.  This view is used in the Subpanel
     * listings.
     */
    public function get_list_view_data()
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::get_list_view_data() has been deprecated in 7.8');
        global $current_language, $app_strings, $app_list_strings, $current_user, $timedate, $locale;
        $product_mod_strings = return_module_language($current_language,"HT_Sales_Service_Products");
        require('modules/HT_Sales_Service_Products/config.php');
    ...
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::build_generic_where_clause() has been deprecated in 7.8');
        $where_clauses = array();
     ...
     * @deprecated
     *
     * product_type is no longer on HT_Sales_Service_Products, it was part of the RLI module
     *
     * map fields if opportunity id is set
     */
    protected function mapFieldsFromOpportunity()
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::mapFieldsFromOpportunity() has been deprecated in 7.8');
    ...
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::convertDateClosedToTimestamp() has been deprecated in 7.8');
    ...
    {
        $GLOBALS['log']->deprecated('HT_Sales_Service_Products::listviewACLHelper() has been deprecated in 7.8');
        ...
        if (ACLController::checkAccess('HT_SS_Quotes', 'view', $is_owner)) {
    ...
    /**
     * Converts (copies) a HT_Sales_Service_Products (QuotedLineItem) to a Revenue Line Item
    ...
    public function convertToRevenueLineItem()
    {
        ...
        if ($rli->likely_case == '0.00' || empty($rli->likely_case)) {
            //undo bad math from ht_ss_quotes.
    ...
    {
        if (!isset($this->ht_sales_service_catalog_id)) {
            // No template to choose from
            return false;
        }
        if (isset($this->fetched_row['ht_sales_service_catalog_id'])
            && $this->ht_sales_service_catalog_id == $this->fetched_row['ht_sales_service_catalog_id']) {
            // Templates are the same, don't do anything
            return false;
        }

        $template = BeanFactory::getBean('HT_Sales_Service_Catalog', $this->ht_sales_service_catalog_id);
    ...
    public function updateCurrencyBaseRate()
    {
        ...
        // load the bundle -> ht_ss_quotes relationship
        $bundle->load_relationship('ht_ss_quotes');

        // get the beans
        $ht_ss_quotes = $bundle->ht_ss_quotes->getBeans();
        $quote = array_pop($ht_ss_quotes);
