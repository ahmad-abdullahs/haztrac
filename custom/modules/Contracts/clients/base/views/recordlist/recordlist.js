...
({
    ...
    duplicateClicked: function (collectionModel) {
        var modelToCopy = app.data.createBean(collectionModel.get('_module'), {id: collectionModel.id});
        modelToCopy.fetch({
            'showAlerts': false,
            'success': _.bind(function (modelToCopy) {
                var prefill = app.data.createBean(collectionModel.get('_module'));

                prefill.copy(modelToCopy);
                this._copyNestedCollections(modelToCopy, prefill);
                modelToCopy.trigger('duplicate:before', prefill);
                prefill.unset('id');
                prefill.unset('contract_number_c');
                app.drawer.open({
                    layout: 'create',
                    context: {
                        create: true,
                        model: prefill,
                        copiedFromModelId: modelToCopy.get('id')
                    }
                ...
                prefill.trigger('duplicate:field', modelToCopy);
