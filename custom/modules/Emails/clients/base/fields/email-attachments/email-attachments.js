...
({
    ...
    initialize: function (options) {
        // This code is added to litsen the link or unlink icons
        this.events = _.extend({}, this.events, {
            'click a[name=advance-relate-unlink]': 'needToLinkEmailAndNotes',
            'click a[name=advance-relate-link]': 'unLinkEmailAndNotes',
        });
    ...
    unLinkEmailAndNotes: function (ele) {
        var emailId = this.model.get('id'),
                type = 'unlink',
                noteId = $(ele.currentTarget).attr('data-id');
        this.relationToEmailNotes(emailId, noteId, type);
    },

    needToLinkEmailAndNotes: function (ele) {
        var emailId = this.model.get('id'),
                type = 'link',
                noteId = $(ele.currentTarget).attr('data-id');
        this.relationToEmailNotes(emailId, noteId, type);
    ...
    relationToEmailNotes: function (emailId, noteId, type) {
        var self = this;
        var message = type == 'link' ? 'Linking to Email as a Note / Attachment.' : 'Unlinking Note / Attachment From Email.';

        app.alert.show('relationToEmailNotes', {
            level: 'process',
            title: message,
            autoClose: false
        });

        var url = app.api.buildURL('relationToEmailNotes', emailId + '/' + noteId + '/' + type);
        app.api.call('update', url, {}, {
            success: function (response) {
                app.alert.dismiss('relationToEmailNotes');

                var linked = (type == 'link') ? 1 : 0;
                var attachments_collection = self.model.get('attachments_collection');
                var attachmentModel = attachments_collection.get(noteId);
                attachmentModel.set('linked', linked);
