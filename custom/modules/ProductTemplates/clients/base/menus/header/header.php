<?php

/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */
$module_name = 'ProductTemplates';
$viewdefs[$module_name]['base']['menu']['header'] = array(
    array(
        'route' => '#' . $module_name . '/create',
        'label' => 'LNK_NEW_PRODUCT',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-plus',
    ),
    array(
        'route' => '#filterProducts',
        'label' => 'LNK_PRODUCT_LIST',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-list',
    ),
//    array(
//        'route'=>'#Manufacturers',
//        'label' =>'LNK_NEW_MANUFACTURER',
//        'acl_action'=>'',
//        'acl_module'=>'',
//        'icon' => 'fa-list',
//    ),
    array(
        'route' => '#createBundle',
        'label' => 'LNK_NEW_BUNDLE_PRODUCT',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-plus',
    ),
    array(
        'route' => '#filterBundleProducts',
        'label' => 'LNK_PRODUCT_LIST_BUNDLE',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-list',
    ),
    array(
        'route' => '#createGroup',
        'label' => 'LNK_NEW_GROUP_PRODUCT',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-plus',
    ),
    array(
        'route' => '#filterGroupProducts',
        'label' => 'LNK_PRODUCT_LIST_GROUP',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-list',
    ),
    array(
        'route' => '#ProductCategories',
        'label' => 'LNK_NEW_PRODUCT_CATEGORY',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-bars',
    ),
    array(
        'route' => '#ProductTypes',
        'label' => 'LNK_NEW_PRODUCT_TYPE',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-bars',
    ),
    array(
        'route' => '#bwc/index.php?module=Import&action=Step1&import_module=ProductTemplates&return_module=ProductTemplates&return_action=index',
        'label' => 'LNK_IMPORT_PRODUCT_CATALOG',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-arrow-circle-o-up',
    ),
);
