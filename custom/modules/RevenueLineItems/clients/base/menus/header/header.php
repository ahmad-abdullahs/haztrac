<?php

/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */
$module_name = 'RevenueLineItems';
$viewdefs[$module_name]['base']['menu']['header'] = array(
    array(
        'route' => '#' . $module_name . '/create',
        'label' => 'LNK_NEW_REVENUELINEITEM',
        'acl_action' => 'create',
        'acl_module' => $module_name,
        'icon' => 'fa-plus',
    ),
    array(
        'route' => '#filterRevenueLineItems',
        'label' => 'LNK_REVENUELINEITEM_LIST',
        'acl_action' => 'list',
        'acl_module' => $module_name,
        'icon' => 'fa-list',
    ),
    array(
        'route' => '#' . $module_name . '/bundleCreate',
        'label' => 'LNK_NEW_BUNDLE_REVENUELINEITEM',
        'acl_action' => 'create',
        'acl_module' => $module_name,
        'icon' => 'fa-plus',
    ),
    array(
        'route' => '#filterBundleRevenueLineItems',
        'label' => 'LNK_REVENUELINEITEM_LIST_BUNDLE',
        'acl_action' => '',
        'acl_module' => '',
        'icon' => 'fa-list',
    ),
    array(
        'route' => '#bwc/index.php?module=Import&action=Step1&import_module=RevenueLineItems&return_module=RevenueLineItems&return_action=index',
        'label' => 'LNK_IMPORT_REVENUELINEITEMS',
        'acl_action' => 'import',
        'acl_module' => $module_name,
        'icon' => 'fa-arrow-circle-o-up',
    ),
);
