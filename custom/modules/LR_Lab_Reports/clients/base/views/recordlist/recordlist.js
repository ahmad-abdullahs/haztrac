...
({
    ...
    duplicateClicked: function (collectionModel) {
        ...
        var labReportModel = app.data.createBean(collectionModel.get('_module'), {id: collectionModel.id});
        labReportModel.fetch({
            'showAlerts': false,
            'success': _.bind(function (labReportModel) {
                var prefill = app.data.createBean(collectionModel.get('_module'));

                prefill.copy(labReportModel);
                this._copyNestedCollections(labReportModel, prefill);
                labReportModel.trigger('duplicate:before', prefill);
                prefill.unset('id');
                ...
                app.drawer.open({
                    ...
                    context: {
                        create: true,
                        model: prefill,
                        copiedFromModelId: labReportModel.get('id')
                    }
                ...
                prefill.trigger('duplicate:field', labReportModel);
