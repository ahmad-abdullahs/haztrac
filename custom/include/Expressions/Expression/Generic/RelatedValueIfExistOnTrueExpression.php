...
class RelatedValueIfExistOnTrueExpression extends GenericExpression {
    ...
    public static function getOperationName() {
        return array("relatedValueIfExistOnTrue");
    }
    ...
    public function evaluate() {
        ...
        $evaluationField = $params[2]->evaluate();
        $ret = '';
        ...
        foreach ($linkField as $bean) {
            if (isset($bean->$relfield)) {
                if (!empty($bean->$evaluationField) && !is_null($bean->$evaluationField) && $bean->$evaluationField) {
                    $ret = $bean->$relfield;
                }
            } else {
                continue;
