...
(function (app) {
    app.events.on('app:init', function () {
        ...
        app.plugins.register('Pagination', ['view'], {
            ...
            // ++ code is update because of the history dashlet on the Accounts detail view, 
            // table like structure is added within the sales and service tab to show the multiple fields.
            _renderRow: function (model) {
                if (model.link) {
                    if (model.link.name == "accounts_sales_and_services_1" && this.name == "account-history") {
                ...
                if (model.link) {
                    if (model.link.name == "accounts_sales_and_services_1" && this.name == "account-history") {
                        this.$el.find("#custom-account-history-table").append($row);
                    } else {
